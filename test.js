  ...
  for (let i = 1; i <= numKids; i++) {
    ...
    if (passType && timeType) {
      ...
      if (passType === "1-Day Pass") {
        ...
        baseCost = discountCost + regularDayCost;

        if (numThreeDaySets > 0) {
          kidBreakdown.push(
            `${numThreeDaySets} x 3-Day Pass (${timeType}): $${discountCost}`
          );
        }
        if (remainingDays > 0) {
          kidBreakdown.push(
        ...
        let promoCost =
          numFourWeekPromos * prices["4-Week Promo"][timeType] * 4;
        let weeklyCost = remainingWeeks * prices.Weekly[timeType];

        baseCost = promoCost + weeklyCost;

        kidBreakdown.push(
          `${numFourWeekPromos} x 4-Week Promo (${timeType}): $${promoCost}`
        );
        if (remainingWeeks > 0) {
          kidBreakdown.push(
            `${remainingWeeks} x Weekly (${timeType}): $${weeklyCost}`
          );
        }
      } else if (passType === "Weekly") {
        baseCost = prices.Weekly[timeType] * numWeeks;
        kidBreakdown.push(`${numWeeks} x Weekly (${timeType}): $${baseCost}`);
      }
    ...
    if (answers.returning === "Yes") {
      let discount = returningDiscount * (selectedDays.length || numWeeks);
      kidCost -= discount;
